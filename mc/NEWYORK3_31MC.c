...
task main()
{
		...
		if(isFlameOff){
			...
			moveforward(16, lowSpd-10); //edited to 16 @matthew chin 3/31/18 2:47
			...
			if(isFlameOff){
				...
				moveforward(18, lowSpd-10); //Maybe move to 16? hop MIGHT be too far.

				if(isFlameOff){
					jobWellDone(2); // another job well done. This is insanely kewl.
				}else{
					// start room#2
					turnRight(90, turnSpd, 0);
					completeStop(1000);
					...
					if(isFlameDetected){
							...
							writeDebugStreamLine("room2 %d %d", (_ticks1_2b*10/rightTicks), (180 - (_ticks1_2b*10/rightTicks) - flameTargetAdj));
							SensorValue[redLed] = 0; // turn on LED
							putOffFlame(); // put off flame
							// how to finish the rest of turn
